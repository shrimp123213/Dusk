...
public class Camcam : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        else if ((bool)Target)
        {
            ...
            {
                transform.position = new Vector3(Mathf.SmoothDamp(transform.position.x, Target.position.x + Target.GetComponent<Character>().Facing * 1.5f, ref velo.x, 0.125f), Mathf.Clamp(Mathf.SmoothDamp(transform.position.y, Target.position.y + 3.5f, ref velo.y, 0.375f), -4.35f, 100f), -10f);
            }
            else
            {
                Vector3 newPos = new Vector3(Mathf.SmoothDamp(transform.position.x, Target.position.x + Target.GetComponent<Character>().Facing * 1.5f, ref velo.x, 0.125f), Mathf.Clamp(Mathf.SmoothDamp(transform.position.y, Target.position.y, ref velo.y, 0.075f), -4.35f, 100f), -10f);
